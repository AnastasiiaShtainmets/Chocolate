...
public class MyGdxGame implements Screen {
	...
	public MyGdxGame(final Chocolate gam) {
		this.game = gam;

		// загрузка изображений для капли и ведра, 64x64 пикселей каждый
		primoImage = new Texture(Gdx.files.internal("1.png"));
		bowlImage = new Texture(Gdx.files.internal("2.png"));

		// загрузка звукового эффекта падающей капли и фоновой "музыки" дождя
		hitSound = Gdx.audio.newSound(Gdx.files.internal("3.mp3"));
		backgroundMusic = Gdx.audio.newMusic(Gdx.files.internal("4.mp3"));
	...
	public void render(float delta) {
		...
		Gdx.gl.glClearColor(2, 3, 0.2f, 1);
		...
		// все капли
		game.batch.begin();
		game.font.draw(game.batch, "Chocolate Collected: " + dropsGathered, 0, 480);
